...
- `Store.DeleteSprint` refuses to delete a completed sprint (permanent history, cascades away its `sprint_issue` rows) but allows deleting a planned or active one — its issues simply fall back to the backlog (`issue.sprint_id` is `ON DELETE SET NULL`).
